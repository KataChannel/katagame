...
curl -s -X POST http://localhost:11101/api/v1/provinces/1/upgrade/farmer \
  -H "Content-Type: application/json" | jq .

echo ""
echo "2. Testing /provinces/1/upgrade/resource WITHOUT auth (expect 401):"
curl -s -X POST http://localhost:11101/api/v1/provinces/1/upgrade/resource \
  -H "Content-Type: application/json" | jq .

echo ""
echo "3. Testing /provinces/1/upgrade/development WITHOUT auth (expect 401):"
curl -s -X POST http://localhost:11101/api/v1/provinces/1/upgrade/development \
  -H "Content-Type: application/json" | jq .
