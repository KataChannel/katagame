...
const handler = async (request: any) => {
  try {
    ...
    const currentLevel = provinceResult.rows[0].resource_level

    // Calculate cost
    const RESOURCE_CONFIG = MVP1_CONFIG.PROVINCE_CONFIG?.UPGRADES?.RESOURCE || {}
    const upgradeCost = (RESOURCE_CONFIG[`level_${currentLevel + 1}`]?.cost) || {
      gold: 800 * (currentLevel + 1),
      wood: 400 * (currentLevel + 1),
    }

    // Check resources
    const resourceResult = await db.query(
      `SELECT resource_type, amount FROM player_resources
      WHERE player_id = $1 AND resource_type IN ('gold', 'wood')`,
      [decoded.playerId]
    )
    ...
    if ((resources.gold || 0) < upgradeCost.gold || (resources.wood || 0) < upgradeCost.wood) {
      return {
        status: 400,
    ...
    try {
      ...
      await db.query(
        `UPDATE player_resources
        SET amount = amount - $1
        WHERE player_id = $2 AND resource_type = 'wood'`,
        [upgradeCost.wood, decoded.playerId]
      )
